...
def get_mission_status(
    ...
    try:
        ...
        # 🔥 Use standardized country filter!
...
@app.get("/api/mission-status-by-company")
def get_mission_status_by_company():
    try:
        conn = get_db_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)

        query = """
        SELECT company, mission_status, COUNT(*) as count
        FROM space_missions
        GROUP BY company, mission_status
        ORDER BY company;
        """
